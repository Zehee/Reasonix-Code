export const zhCN = {
  ...
  overview: {
    ...
    standaloneDesc:
      "只读磁盘视图。在 reasonix-code code 内启动 /dashboard 以获取实时会话状态、MCP 和工具。",
    cockpit: "驾驶舱",
    balance: "余额",
    tokens7d: "tokens · 7 天",
    cacheHit: "缓存命中",
    ...
    budget: "预算",
    currentSession: "当前会话",
    noSession: "无活跃会话 — 在 reasonix-code code 内执行 /dashboard 进行连接。",
    promptTok: "提示 tokens",
  ...
  permissions: {
    ...
    standaloneWarning:
      "修改操作需要在活跃的 reasonix-code code 会话内执行 /dashboard — 独立模式的 reasonix-code dashboard 无法确定要编辑哪个项目的允许列表。",
  },
  mcp: {
    loading: "加载 MCP…",
    ...
    saved: "已保存",
    savedRestart: "已保存 — 重启 reasonix-code code 以桥接此服务器",
    ...
    whyUnbridged: "为什么未桥接？",
    whyUnbridgedDesc:
      "此规格存在于您的 config.json 中，但未桥接到实时会话。MCP 服务器在 reasonix-code code 启动时连接；仪表盘本身无法生成子进程。",
    whyUnbridgedHint: "激活方法：重启 reasonix-code code，然后刷新此仪表盘。",
    bridgeFailed: "桥接失败",
    bridgeFailedTitle: "桥接失败 · 在配置中",
    bridgeFailedHint:
      "已尝试桥接但失败。常见原因：URL 错误、需要鉴权、上游 404 / 5xx、本地命令缺失。修复后重启 reasonix-code code 重试。",
  ...
  hooks: {
    ...
    colScript: "脚本",
    noProject: "无活跃项目 — 在 reasonix-code code 中打开 /dashboard 以编辑项目钩子。",
    saveReload: "保存并重载",
    discard: "放弃更改",
  ...
  system: {
    ...
    built: "● 已构建",
    none: "— 无",
    runIndex: "运行 reasonix-code index 以构建",
    usageLog: "用量日志",
    backgroundJobs: "后台任务",
    noSession: "— 无会话",
    running: "{count} 个运行中",
