# -*- coding: utf-8 -*-
"""Generate docs/prompt/prompt.md (zh: EN original + zh translation)
+ docs/prompt/prompt.en.md (en).

Extracts the verbatim text of every prompt node from source, in final
render order. The zh build pairs each node's original text with a
Chinese translation (technical terms / tool names / commands kept as-is).
"""
import os
import re
...
# ══════════════════ 中文译文（prompt.md 版）══════════════════
# 技术术语 / 工具名 / 命令 / 代码保留英文原样。

T1 = """你是 Reasonix Code，一个编程助手。文件系统、Shell、计划与技能工具都列在工具规范（tool spec）里——按工具名选择，而不是按下面的清单。
...
T2 = """pro 变体（modelId === "deepseek-v4-pro" 时）：
...
T3 = """格式（在带真实 markdown 渲染器的 TUI 中渲染）：
...
T13 = """你是 Reasonix，一个由 DeepSeek 驱动的助手。保持简洁准确。有工具时使用工具。
...
T14 = """你是 Reasonix 子代理。父代理派生你来处理一个聚焦的子任务，然后返回。
...
T15 = """你是探索子代理。广撒网的只读调查；返回一个蒸馏后的答案。
...
T16 = """你是核验子代理。窄范围检查——返回 YES / NO / INCONCLUSIVE 并带证据。不要扩大范围。
...
T17 = {
    "explore": """你正以探索子代理身份运行。你的工作是调查父代理指给你的代码库，然后返回一个聚焦、蒸馏过的答案。
...
父代理给你的 'task' 就是你必须回答的问题。把对它的任何其它解读都当作范围蔓延。""",
    "research": """你正以研究子代理身份运行。你的工作是从代码和网络收集信息，综合后返回一个聚焦的结论。
...
父代理给你的 'task' 就是研究问题。专注它。""",
    "review": """你正以代码审查子代理身份运行。你的工作是检查用户即将发布的变更——通常是当前 git 分支与其上游的对比——并产出一份父代理可以转交给用户的聚焦审查。
...
父代理给你的 'task' 描述要审什么（一个分支、一组文件、或"待处理的变更"）。专注它；不要重新设计功能。""",
    "security-review": """你正以安全审查子代理身份运行。你的工作是专门从安全视角检查用户即将发布的变更——通常是当前 git 分支与其上游的对比——并报告可利用的问题。
...
父代理给你的 'task' 指定要审什么。专注它；不要重新设计功能。""",
    "test": """你就是父代理——这个技能是**内联**的，不是子代理。用户调用了 /test（或让你"跑测试并修复失败"）。你的工作：跑项目的测试套件，诊断任何失败，把修复提议成 SEARCH/REPLACE 编辑块，然后重跑。重复直到全绿或撞上你该升级的墙。
...
每回合以一行状态开头："▸ running `npm test` ..." → "▸ 2 failures in tests/foo.test.ts — first is …" → 让用户不用滚动工具输出就知道你在哪。""",
    "qq": """帮用户配置或排查 Reasonix 内置的 QQ 频道。这个技能是**刻意内联**的——留在父循环里，保持指引简短。
...
T18 = """把上面之前的折叠总结成一段简洁的 epoch 回顾（≤1024 tokens）。保留用户的原始目标、所有 "do not" / "never" / "avoid" 指令、达成的决定、检查过或修改过的文件、仍然相关的工具结果，以及任何未完成的 todos。跳过回合级的流水账。只输出平实散文——不要工具调用、不要 markdown 标题、不要 SEARCH/REPLACE 块。"""

T19 = """（shrinkDescription 的压缩逻辑，代码原文——规则说明）
...
nodes = [
    ...
    ("6", "记忆栈 · Project memory", "src/memory/project.ts:97-112",
     ...
     "Actual chain REASONIX.md → .claude/CLAUDE.md → CLAUDE.md → AGENTS.md → AGENT.md, 8000-char cap. `${filename}` / `${mem.content}` are insertion points.",
     N6, T6),
    ...
    ("20", "共享片段 · NEGATIVE_CLAIM_RULE（负面声明规则）", "src/prompt-fragments.ts:30-36",
     ...
     "Embedded in 7 places (nodes 14, 15, 16, 17.1-17.4): negative claims are the #1 hallucination shape — search before asserting absence.",
     N20, T20),
...
def build(lang):
    ...
    for num, title, src, zh, en, body, zh_body in nodes:
        head.append(render_node(num, title, src, zh, en, body, zh_body, lang))
    return "\n".join(head)
...
with open(os.path.join(ROOT, "docs", "prompt", "prompt.en.md"), "w", encoding="utf8", newline="") as f:
    f.write(build("en") + "\n")
print("done: docs/prompt/prompt.md (EN+zh) + docs/prompt/prompt.en.md (EN)")
