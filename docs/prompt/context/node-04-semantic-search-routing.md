...
> 这段文字（`SEMANTIC_SEARCH_ROUTING`）仅在 `hasSemanticSearch` 为真时附加到节点 1 的末尾（即 `semantic_search` 工具已注册、ollama 索引可达）。它告诉模型该先调哪个搜索工具。
>
> 原文（翻译说明）——
>
> > # 搜索路由
...
> >
> > 如果 `semantic_search` 没有返回有用的东西（低分、离题），再回退到 `grep`。不要反着来——用 grep 去搜改写过的问句会浪费整个回合。
