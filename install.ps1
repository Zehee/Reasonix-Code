...
.PARAMETER AddToPath
    If set, adds the install directory to the current user's PATH.
.PARAMETER Force
...
# Bun --compile produces a single binary per platform; on Windows it's always .exe.
# For cross-platform support the release could carry arch suffixes in the future.
$assetName = "$ExeName"
...
Write-Host "Done! Run 'reasonix-code --help' to get started." -ForegroundColor Green
