...
export interface AppProps {
   ...
   * apply them to disk under `rootDir`. Set by `reasonix-code code`. The
...
function AppInner({
  ...
  const startWalkthrough = useCallback((): string => {
    if (!codeMode) {
      return "/walk is only available inside `reasonix-code code`.";
    }
      ...
      <TickerProvider disabled={tickerSuspended}>
        ...
        */}
                ...
                <LiveActivityArea
                  ...
                  suppressPlanLiveRow={planActive}
              ...
              {pendingPlan && !stagedInput && !pendingReviseEditor ? (
                ...
                <PlanReviseEditor
                  ...
                  onAccept={() => handleReviseConfirm("accept")}
                  onCancel={() => setPendingReviseEditor(null)}
                />
              ) : stagedChoiceCustom ? (
                <PlanRefineInput
                  mode="choice-custom"
