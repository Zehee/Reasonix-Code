...
/** Resolve dashboard/ across tsx-dev, tsup-bundled, and standalone-binary layouts. */
function resolveAssetDir(): string {
  const here = dirname(fileURLToPath(import.meta.url));
  // Try a few candidates; the first existing one wins.
  //
  // When running via tsx (dev):
  //   src/server/assets.ts → ../../dashboard → project-root/dashboard
  //
  // When running from the tsup bundle in dist/:
  //   dist/ (library) → ./dashboard
  //   dist/cli/       → ../dashboard → dist/dashboard
  //
  // When running from the standalone binary (bun build --compile),
  // import.meta.url resolves to a temp extraction directory on a
  // different drive (e.g. B:\).  In that case the real dashboard/
  // lives next to the executable (process.execPath).
  const execDir = dirname(process.execPath);
  const candidates = [
    join(here, "..", "..", "dashboard"),
    join(here, "..", "dashboard"),
    join(here, "dashboard"),
    // Standalone binary: dashboard/ is alongside the .exe
    join(execDir, "dashboard"),
  ];
