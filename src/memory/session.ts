...
/**
 * Read the minimum state needed to decide fast-path vs slow-path append
 * and to compute nextTurnId.
 *
 *   - Fast-path: live JSONL exists, non-empty, parses cleanly at the
 *     tail. We only need the tail line because turnIds are monotonic
 *     (every append goes through this code path), so max(turnId) on the
 *     full log equals the tail line's turnId. Resolve the tail with the
 *     O(1) byte-scan readTailMessages(1).
 *   - Slow-path: live missing / empty / corrupt / .bak-recovered → fall
 *     back to loadSessionMessages (full reload, which already handles
 *     .bak fallback).
 *
 * The caller's job is then to compare the tail-only signal against the
